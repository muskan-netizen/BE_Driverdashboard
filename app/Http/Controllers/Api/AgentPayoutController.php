...
class AgentPayoutController extends BaseController{
    ...
    public function agentPayoutRequestCreate(Request $request, $id){
        try{
            ...
            DB::beginTransaction();
            $agent = Agent::where('id', $id)->lockForUpdate()->first();
            if (!$agent) {
                DB::rollBack();

                return $this->error(__('Agent not found'), 404);
            }
            $agent_id = $agent->id;
            ...
            $n = DB::update(
                'UPDATE agents SET available_funds = available_funds - ? WHERE id = ? AND available_funds >= ?',
                [$amt, $agent_id, $amt]
            );
            if ($n !== 1) {
                DB::rollBack();
    ...
    public function agentPayoutDetails(Request $request)
    {
        ...
        $available_funds = number_format(round((float) ($agent->available_funds ?? 0), 2), 2, '.', ',');
